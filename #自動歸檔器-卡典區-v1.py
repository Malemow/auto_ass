...
pt_Print  = '/Users/linlifan/desktop/water_box/' #抓取絕對路徑
for root,dirs,files in walk(pt_Print + '卡典集散區' ): #執行此路徑內所有檔案
  for i in files:
    ...
    if pt_suffix in suffix and pt_chreck in "-" :
      ...
      shutil.move(pt_fider,pt_Print + dir_yers + '年/' + dir_mon + '月/'  + pt_name ) #移動檔案到此單號資料夾
