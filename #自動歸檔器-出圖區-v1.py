...
int_error = 0
pt_Print =  '/Volumes/FileServer2/Sand box/出圖區/'
for root,dirs,files in walk(pt_Print + '圖檔集散地' ):
  for i in files:
    ...
    suffix    = ['.pdf' , '.tif' , '.PDF' , '.TIF' ,'.tiff' , '.TIFF']  #把pdf , tif 定義為suffix方便檢視
    ...
    if pt_suffix in suffix and pt_chreck in "-":
      ...
      shutil.move(pt_fider,pt_Print + dir_yers + '年/' + dir_mon + '月/' + dir_name + '/' + pt_name )

      print ('成功將', pt_name , '移動至', dir_name , '資料夾')
